...
//  gl_FragColor = vec4(d*20,0,0,1);
  ...
  vec3 n = normalize(normal+0.7*bump);
  //vec3 n = normalize(normal);
  vec3 h = (l+v)/2.0;
  float nh = max(dot(n, h),0.0);
  float ks = 0*pow(nh,90.1);
  vec3 reflection = (0.95*max(dot(l,n),0.0))*diffuse*lightcolor
                    +ks*specular*diffuse+ambient;

  float fresnel = pow(1.0 - dot(n, v), 5.0);  // Fresnel term for subsurface scattering
  float ssIntensity = 0.2;  // Adjust the subsurface scattering intensity
  vec3 subsurface = ssIntensity * (1.0 - fresnel) * diffuse;
